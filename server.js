...
const mailFunction = require('./Util/mailer')
    ...
    try {
        ...
        if(user){
            if(user.Email == req.body.Email){
                let resetToken = jwt.sign({_id:user._id},process.env.SECRET,{expiresIn:"10m"})
        ...
        if(data){
            ...
            if(verifyToken){
                try {
                    ...
                    let hash = await bycrypt.hash(req.body.Password,salt)
                    req.body.Password = hash
                    await Auth.findByIdAndUpdate(data._id,{$set:{Password:req.body.Password,resetToken:""}})
...
app.post('/customers/:id',async function(req,res){
    try {
        let data = await Customers.findOneAndUpdate({_id:req.params.id},{$set:{Status:req.body.Status}})
        if(data){
            res.status(200).json({message:"Employee Added"})
        }
    } catch (error) {
